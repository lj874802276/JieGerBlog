...
import { Badge, StepItem, Steps, TabGroup, Timeline, TimelineItem } from '@components/jiegerbolg-mdx';

:::tip
[JieGerBolg](https://github.com/lj874802276/JieGerBolg) 支持 `MDX` 和 `Markdown` 两种类型的文章，你可以在文章中混合使用两种格式，如果没有特别复杂内容和需求，推荐使用 Markdown 格式就够了。
:::
...
## JieGerBolg 内置的专属 MDX UI 组件
这里列出 JieGerBolg 内置的 MDX UI 组件，方便在 MDX 文章中直接使用。
